...
public record BcOptionConfig : IBcExtensionData
{
    /// <summary>
    ...
    public class ObjectValueConverter : JsonConverter<object?>
    {
        public override object? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            {
                // Try to read as int64 first, then double if needed
            ...
            // Check if the JSON token is a boolean
            if (reader.TokenType == JsonTokenType.True || reader.TokenType == JsonTokenType.False)
            {
                return reader.GetBoolean();
            }
            // For objects and arrays, deserialize using JsonElement
